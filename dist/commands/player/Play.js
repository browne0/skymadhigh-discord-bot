...
export default (async msg => {
  if (msg.content === `${prefix}play`) {
    ...
    if (queueList[msg.guild.id].length === 0) {
      await msg.reply('The list of songs is currently empty. You can play a new song with the following command:');
    ...
    if (parseInt(secondWord, 10)) {
      ...
      if (index <= queueList[msg.guild.id].length && index >= 1) {
        if (msg.guild.voiceConnection) {
          await utils.replaceFirstSong(msg, queueList[msg.guild.id][index - 1].url, false);
          ...
          const connection = await join(msg);
          utils.dispatchSong(connection, msg, queueList[msg.guild.id][index - 1]);
          await utils.replaceFirstSong(msg, queueList[msg.guild.id][index - 1].url);
        ...
        await msg.reply('Please choose a song from the queue, using `!play [position in list].` If you want to play the current song, just type `!play`.');
      }
    } else {
      ...
      if (utils.batchIncludes(ytStrings, url)) {
        ...
        if (msg.guild.voiceConnection) {
          if (queueList[msg.guild.id].length === 0) {
            ...
            await utils.dispatchSong(msg.guild.voiceConnection, msg, queueList[msg.guild.id][0]);
            return;
          }
