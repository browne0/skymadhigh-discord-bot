...
export default (async (connection, message, song) => {
  await message.channel.send(`Now Playing: **${song.title}** from **${song.user}**.`);
  ...
  collector.on('collect', async m => {
    if (m.content.startsWith(`${prefix}pause`)) {
      ...
      if (m.content === `${prefix}volume`) {
        ...
        await message.channel.send(`:speaker: Current volume: ${currentVol * 100}/100.`);
        return;
      }
      ...
      await message.channel.send(`:clock1: Time: ${Math.floor(dispatcher.time / 60000)}:${Math.floor(dispatcher.time % 60000 / 1000) < 10 ? `0${Math.floor(dispatcher.time % 60000 / 1000)}` : Math.floor(dispatcher.time % 60000 / 1000)}`);
    }
  });
  dispatcher.on('end', async reason => {
    ...
    if (reason === 'stop') {
      ...
      const index = queueList[message.guild.id].findIndex(item => item.id === song.id);
